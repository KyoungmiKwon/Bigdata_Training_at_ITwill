...
def stock_preprocessing(data, t_view=20, DATE = 'Date', TARGET = 'Close'):
    global col_num
    date_j = []
    data_j = []
    label_j = []
...
#######################################################################################################################
                        ...
                        scaler.fit(samsung_stock[scale_cols][t_view:-cut])
...
#####
                    ...
                    pred_rev = reverse_min_max_scaling(samsung_stock['Close'][t_view:-cut], pred)
                    y_rev = reverse_min_max_scaling(samsung_stock['Close'][t_view:-cut], Label_test)
                    pd.DataFrame(pred).to_excel('stock_pred.xlsx')
                    pd.DataFrame(pred_rev).to_excel('stock_pred_rev.xlsx')
                    pd.DataFrame(Label_test).to_excel('stock_label_orig.xlsx')
                    pd.DataFrame(y_rev).to_excel('stock_label_rev.xlsx')
                    ...
                    plt.legend()
                    plt.savefig(f'{t_view}_{col_num}_{cut}_l{lstm_num}_d{dense_num}.png', dpi=200)